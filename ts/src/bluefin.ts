...
export default class bluefin extends Exchange {
  ...
  async fetchTicker(symbol: string, params = {}): Promise<Ticker> {
    ...
    const response = await this.exchangeGetV1ExchangeTicker(
  ...
  async fetchOrderBook(
    ...
    const response = await this.exchangeGetV1ExchangeDepth(
      this.extend(request, params),
    );
  ...
  async fetchTrades(
    ...
    const response = await this.exchangeGetV1ExchangeTrades(
      this.extend(request, params),
    );
    const result: Trade[] = [];
    for (let i = 0; i < response.length; i++) {
  ...
  async fetchOHLCV(
    ...
    const response = await this.exchangeGetV1ExchangeCandlesticks(
      this.extend(request, params),
    );
    const result: OHLCV[] = [];
    for (let i = 0; i < response.length; i++) {
  ...
  async fetchFundingRateHistory(
    ...
    const response = await this.exchangeGetV1ExchangeFundingRateHistory(
      this.extend(request, params),
    );
    ...
    return this.safeMarketStructure({
      ...
      "limits": {
        "amount": {
          ...
          "max": this.parseE9(
            this.safeString(market, "maxLimitOrderQuantityE9"),
          ),
        },
        "price": {
  ...
  parseTicker(ticker: Dict, market: Market = undefined): Ticker {
    ...
    const symbol = (bluefinSym !== undefined)
      ? this.ccxtSymbol(bluefinSym)
      : undefined;
    const last = this.parseE9(
      this.safeString(ticker, "lastPriceE9"),
    );
    const mark = this.parseE9(
      this.safeString(ticker, "markPriceE9"),
    );
    const index = this.parseE9(
      this.safeString(ticker, "oraclePriceE9"),
    );
    const high = this.parseE9(
      this.safeString(ticker, "highPrice24hrE9"),
    );
    const low = this.parseE9(
      this.safeString(ticker, "lowPrice24hrE9"),
    );
    const open = this.parseE9(
      this.safeString(ticker, "openPrice24hrE9"),
    );
    const close = last;
    const bid = this.parseE9(
      this.safeString(ticker, "bestBidPriceE9"),
    );
    const ask = this.parseE9(
      this.safeString(ticker, "bestAskPriceE9"),
    );
    const bidVolume = this.parseE9(
      this.safeString(ticker, "bestBidQuantityE9"),
    );
    const askVolume = this.parseE9(
      this.safeString(ticker, "bestAskQuantityE9"),
    );
    const baseVolume = this.parseE9(
      this.safeString(ticker, "volume24hrE9"),
    );
    const quoteVolume = this.parseE9(
      this.safeString(ticker, "quoteVolume24hrE9"),
    );
    const change = this.parseE9(
      this.safeString(ticker, "priceChange24hrE9"),
    );
    const percentRaw = this.parseE9(
      this.safeString(ticker, "priceChangePercent24hrE9"),
    );
  ...
  parseTrade(trade: Dict, market: Market = undefined): Trade {
    ...
    const symbol = (bluefinSym !== undefined)
      ? this.ccxtSymbol(bluefinSym)
      : undefined;
    const price = this.parseE9(
      this.safeString(trade, "priceE9"),
    );
    const amount = this.parseE9(
      this.safeString(trade, "quantityE9"),
    );
    const cost = this.parseE9(
      this.safeString(trade, "quoteQuantityE9"),
    );
    const side = this.parseOrderSide(
      this.safeString(trade, "side"),
    );
    const timestamp = this.safeInteger(
      trade,
      "executedAtMillis",
  ...
  parseOrder(order: Dict, market: Market = undefined): Order {
    ...
    const side = this.parseOrderSide(
      this.safeString(order, "side"),
    );
    const price = this.parseE9(
      this.safeString(order, "priceE9"),
    );
    const amount = this.parseE9(
      this.safeString(order, "quantityE9"),
    );
    const filled = this.parseE9(
      this.safeString(order, "filledQuantityE9"),
    );
  ...
  parsePosition(position: Dict, market: Market = undefined): Position {
    ...
    const contracts = this.parseE9(
      this.safeString(position, "sizeE9"),
    );
    const entryPrice = this.parseE9(
      this.safeString(position, "avgEntryPriceE9"),
    );
    const markPrice = this.parseE9(
      this.safeString(position, "markPriceE9"),
    );
    const liquidationPrice = this.parseE9(
      this.safeString(position, "liquidationPriceE9"),
    );
    const notional = this.parseE9(
      this.safeString(position, "notionalValueE9"),
    );
    const unrealizedPnl = this.parseE9(
      this.safeString(position, "unrealizedPnlE9"),
    );
    const initialMargin = this.parseE9(
      this.safeString(position, "marginRequiredE9"),
    );
    const maintenanceMargin = this.parseE9(
      this.safeString(position, "maintenanceMarginE9"),
    );
    const leverage = this.parseE9(
      this.safeString(position, "clientSetLeverageE9"),
    );
    const isIsolated = this.safeBool(position, "isIsolated");
    const marginMode = isIsolated ? "isolated" : "cross";
    const collateral = isIsolated
      ? this.parseE9(
        this.safeString(position, "isolatedMarginE9"),
      )
      : initialMargin;
    const timestamp = this.safeInteger(
      position,
  ...
  parseBalance(response: Dict): Balances {
    ...
    for (let i = 0; i < assets.length; i++) {
      ...
      account["total"] = this.parseE9(
        this.safeString(asset, "quantityE9"),
      );
      account["free"] = this.parseE9(
        this.safeString(asset, "effectiveBalanceE9"),
      );
      result[code] = account;
    }
  ...
  parseFundingRateHistory(
    ...
    const symbol = (bluefinSym !== undefined)
      ? this.ccxtSymbol(bluefinSym)
      : undefined;
    const fundingRate = this.parseE9(
      this.safeString(entry, "fundingRateE9"),
    );
    const timestamp = this.safeInteger(
      entry,
      "fundingTimeAtMillis",
