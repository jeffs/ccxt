...
class bluefin(Exchange, ImplicitAPI):
    ...
    response = self.exchangeGetV1ExchangeTicker(
    ...
    if limit is not None:
      request["limit"] = limit
    response = self.exchangeGetV1ExchangeDepth(
      self.extend(request, params),
    )
    ...
    if limit is not None:
      request["limit"] = limit
    response = self.exchangeGetV1ExchangeTrades(
      self.extend(request, params),
    )
    result: List[Trade] = []
    for i in range(0, len(response)):
    ...
    if limit is not None:
      request["limit"] = limit
    response = self.exchangeGetV1ExchangeCandlesticks(
      self.extend(request, params),
    )
    result: List[OHLCV] = []
    for i in range(0, len(response)):
    ...
    if limit is not None:
      request["limit"] = limit
    response = self.exchangeGetV1ExchangeFundingRateHistory(
      self.extend(request, params),
    )
