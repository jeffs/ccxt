...
class bluefin(Exchange, ImplicitAPI):
    ...
    response = await self.exchangeGetV1ExchangeTicker(
    ...
    if limit is not None:
      request["limit"] = limit
    response = await self.exchangeGetV1ExchangeDepth(
      self.extend(request, params),
    )
    ...
    if limit is not None:
      request["limit"] = limit
    response = await self.exchangeGetV1ExchangeTrades(
      self.extend(request, params),
    )
    result: List[Trade] = []
    for i in range(0, len(response)):
    ...
    if limit is not None:
      request["limit"] = limit
    response = await self.exchangeGetV1ExchangeCandlesticks(
      self.extend(request, params),
    )
    result: List[OHLCV] = []
    for i in range(0, len(response)):
    ...
    if limit is not None:
      request["limit"] = limit
    response = await self.exchangeGetV1ExchangeFundingRateHistory(
      self.extend(request, params),
    )
    ...
    raise new Error("createOrder not implemented")
    ...
    raise new Error("cancelOrder not implemented")
    ...
    raise new Error("cancelOrders not implemented")
    ...
    raise new Error("setLeverage not implemented")
    ...
    raise new Error("setMarginMode not implemented")
    ...
    raise new Error("withdraw not implemented")
    ...
    return self.safe_market_structure({
      ...
      "limits": {
        "amount": {
          ...
          "max": self.parse_e9(
            self.safe_string(market, "maxLimitOrderQuantityE9"),
          ),
        },
        "price": {
    ...
    symbol = self.ccxt_symbol(bluefinSym)
      if (bluefinSym is not None) else None
    last = self.parse_e9(
      self.safe_string(ticker, "lastPriceE9"),
    )
    mark = self.parse_e9(
      self.safe_string(ticker, "markPriceE9"),
    )
    index = self.parse_e9(
      self.safe_string(ticker, "oraclePriceE9"),
    )
    high = self.parse_e9(
      self.safe_string(ticker, "highPrice24hrE9"),
    )
    low = self.parse_e9(
      self.safe_string(ticker, "lowPrice24hrE9"),
    )
    open = self.parse_e9(
      self.safe_string(ticker, "openPrice24hrE9"),
    )
    close = last
    bid = self.parse_e9(
      self.safe_string(ticker, "bestBidPriceE9"),
    )
    ask = self.parse_e9(
      self.safe_string(ticker, "bestAskPriceE9"),
    )
    bidVolume = self.parse_e9(
      self.safe_string(ticker, "bestBidQuantityE9"),
    )
    askVolume = self.parse_e9(
      self.safe_string(ticker, "bestAskQuantityE9"),
    )
    baseVolume = self.parse_e9(
      self.safe_string(ticker, "volume24hrE9"),
    )
    quoteVolume = self.parse_e9(
      self.safe_string(ticker, "quoteVolume24hrE9"),
    )
    change = self.parse_e9(
      self.safe_string(ticker, "priceChange24hrE9"),
    )
    percentRaw = self.parse_e9(
      self.safe_string(ticker, "priceChangePercent24hrE9"),
    )
    ...
    symbol = self.ccxt_symbol(bluefinSym)
      if (bluefinSym is not None) else None
    price = self.parse_e9(
      self.safe_string(trade, "priceE9"),
    )
    amount = self.parse_e9(
      self.safe_string(trade, "quantityE9"),
    )
    cost = self.parse_e9(
      self.safe_string(trade, "quoteQuantityE9"),
    )
    side = self.parse_order_side(
      self.safe_string(trade, "side"),
    )
    timestamp = self.safe_integer(
      trade,
      "executedAtMillis",
    ...
    price = self.parse_e9(
      self.safe_string(order, "priceE9"),
    )
    amount = self.parse_e9(
      self.safe_string(order, "quantityE9"),
    )
    filled = self.parse_e9(
      self.safe_string(order, "filledQuantityE9"),
    )
    ...
    contracts = self.parse_e9(
      self.safe_string(position, "sizeE9"),
    )
    entryPrice = self.parse_e9(
      self.safe_string(position, "avgEntryPriceE9"),
    )
    markPrice = self.parse_e9(
      self.safe_string(position, "markPriceE9"),
    )
    liquidationPrice = self.parse_e9(
      self.safe_string(position, "liquidationPriceE9"),
    )
    notional = self.parse_e9(
      self.safe_string(position, "notionalValueE9"),
    )
    unrealizedPnl = self.parse_e9(
      self.safe_string(position, "unrealizedPnlE9"),
    )
    initialMargin = self.parse_e9(
      self.safe_string(position, "marginRequiredE9"),
    )
    maintenanceMargin = self.parse_e9(
      self.safe_string(position, "maintenanceMarginE9"),
    )
    leverage = self.parse_e9(
      self.safe_string(position, "clientSetLeverageE9"),
    )
    isIsolated = self.safe_bool(position, "isIsolated")
    marginMode = "isolated" if isIsolated else "cross"
    collateral = self.parse_e9(
        self.safe_string(position, "isolatedMarginE9"),
      )
    ...
    for i in range(0, len(assets)):
      ...
      account["total"] = self.parse_e9(
        self.safe_string(asset, "quantityE9"),
      )
      account["free"] = self.parse_e9(
        self.safe_string(asset, "effectiveBalanceE9"),
      )
      result[code] = account
    ...
    symbol = self.ccxt_symbol(bluefinSym)
      if (bluefinSym is not None) else None
    fundingRate = self.parse_e9(
      self.safe_string(entry, "fundingRateE9"),
    )
    timestamp = self.safe_integer(
      entry,
      "fundingTimeAtMillis",
    ...
    #
    raise new Error("authenticate not implemented")
    ...
    raise new Error("refreshAccessToken not implemented")
    ...
    raise new Error("getAccessToken not implemented")
    ...
    #
    raise new Error("signRequest not implemented")
